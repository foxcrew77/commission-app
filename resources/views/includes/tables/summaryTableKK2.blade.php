<div class="flex flex-col w-full mb-2">
    <div class="text-sm py-2 font-bold tracking-wide text-gray-600 uppercase border dark:border-gray-600 bg-gray-50 dark:text-gray-400 dark:bg-gray-700">
        @foreach ($totalKK2MonthlyCommission[0] as $comm)
        {{ $outlet }} (Total: $ {{ $comm }})
        @endforeach
    </div>
    <div class="content flex flex-row w-full">
        <div class="column w-full">
            <table class="w-full whitespace-no-wrap" style="border-collapse: collapse;"> 
                ...
                >
                <?php  $sumDriver = 0 ?>
                @foreach ($KK2driverMonthlyCommission as $comm)
        ...
        <div class="column w-full">
            <table class="w-full whitespace-no-wrap"> 
                ...
                {{-- first row --}}
                <?php  $sumWorkman = 0 ?>
                @foreach ($KK2workmanMonthlyCommission as $comm)
                    <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3 font-semibold border">
                            {{ $comm->Workman }}
                        </td>
                    ...
                    <?php  $sumWorkman += $comm->Commission ?>
                @endforeach
                <tr>
                    <td class="px-4 py-3 font-bold border dark:text-gray-400">Total (Workman):</td>
                    <td class="px-4 py-3 text-sm font-semibold dark:text-gray-400">{{ $sumWorkman }}</td>
                </tr>
                </tbody>
            </table>
        </div>
